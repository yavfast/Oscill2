...
> - 2026-07-17 — **Truncated-frame guard**: `_read_resp` now rejects an incomplete OBEX packet — the 3-byte header declares total length, so a complete read returns exactly `length-3` body bytes; a short body (mid-packet read-timeout / incomplete chunk) raises `IOError` so the partial frame is skipped by the acquisition loop instead of being parsed/plotted. (The documented `0xB0` checksum is optional and this device does not emit it, so the self-framing declared length is the completeness signal.)
